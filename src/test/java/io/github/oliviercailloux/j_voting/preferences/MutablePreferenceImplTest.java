...
class MutablePreferenceImplTest {
	...
	@Test
	void testGivenVoter() {
		MutablePreferenceImpl pref = MutablePreferenceImpl.given(Voter.createVoter(1));
	...
	void testPreferenceGraphMaker() {
		...
		graph.putEdge(a2, a3);
		graph.putEdge(a4, a5);
		assertEquals(graph, MutablePreferenceImpl.preferenceGraphMaker(setTest));
	}
	...
	void testGivenPreference() {
		...
		setTest.add(listTest2);
		MutableGraph<Alternative> graph = GraphBuilder.directed().allowsSelfLoops(true).build();
		MutablePreferenceImpl pref = MutablePreferenceImpl.given(graph, Voter.createVoter(1));
